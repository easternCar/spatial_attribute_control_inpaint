...
def save_masks(mask, dir):
    img_num = mask.shape[0]
    mask = np.transpose(mask, (0, 2, 3, 1))

    for i in range(img_num):
        cv2.imwrite(dir + '/' + str(i).zfill(3) + '.png' ,mask[i])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--path', type=str, default='mask_code/rand_masks')
    ...
    bm = BatchRandomMask(args.num, 128)       # d
    save_masks(bm, args.path)
